...
# We use named groups for clarity and compatibility with ExplicitCapture
$pattern = '(?<prefix>\basync\s+[\w\s<>,\[\]]*?(?:Task|ValueTask)[\w\s<>,\[\]]*?\s+)(?<name>\w+)(?<!Async)(?<suffix>\s*\()'
...
foreach ($match in $matches) {
    ...
    $newMethodName = "${methodName}_Async"
...
if ($PSCmdlet.ShouldProcess($FilePath, "Save changes to file")) {
    $evaluator = { param($m) 
        $existingName = $m.Groups['name'].Value
        $newName = "${existingName}_Async"
        Write-Host "  Renaming '$existingName' -> '$newName'" -ForegroundColor Green
        return $m.Groups['prefix'].Value + $newName + $m.Groups['suffix'].Value
    }
    
    $finalContent = [regex]::Replace($originalContent, $pattern, $evaluator, "ExplicitCapture")
