...
namespace MTM_Receiving_Application.Module_Receiving.Services
{
    ...
    public class Service_ReceivingValidation : IService_ReceivingValidation
    {
        ...
        private static readonly Regex _qualityHoldRegex = new Regex(
            @"(MMFSR|MMCSR)",
            RegexOptions.IgnoreCase
        );
        ...
        public Model_ReceivingValidationResult ValidateNumberOfLoads(int numLoads)
        {
            ...
            {
                return Model_ReceivingValidationResult.Error(
                    $"Number of loads must be at least {minimumLoads}"
                );
            }
            ...
            {
                return Model_ReceivingValidationResult.Error(
                    $"Number of loads cannot exceed {maximumLoads}"
                );
            }
        ...
        public Model_ReceivingValidationResult ValidateWeightQuantity(decimal weightQuantity)
        {
            ...
            {
                return Model_ReceivingValidationResult.Error(
                    "Weight/Quantity must be greater than 0"
                );
            }
            ...
            {
                return Model_ReceivingValidationResult.Error(
                    $"Weight/Quantity cannot be less than {minimumQuantity}"
                );
            }
            ...
            {
                return Model_ReceivingValidationResult.Error(
                    $"Weight/Quantity cannot exceed {maximumQuantity}"
                );
            }
        ...
        public Model_ReceivingValidationResult ValidateHeatLotNumber(string heatLotNumber)
        {
            ...
            {
                return Model_ReceivingValidationResult.Error(
                    "Heat/Lot number cannot exceed 50 characters"
                );
            }
        ...
        )
        {
            ...
            var existsResult = await _inforVisualService.LocationExistsAsync(
                normalizedLocation,
                warehouseCode
            );
            ...
            {
                return Model_ReceivingValidationResult.Error(
                    existsResult.ErrorMessage ?? "Unable to validate location in Infor Visual"
                );
            }
            ...
            return existsResult.Data
                ? Model_ReceivingValidationResult.Success()
                : Model_ReceivingValidationResult.Error(
                    $"Location '{normalizedLocation}' was not found in warehouse {warehouseCode}"
                );
        }
        ...
        )
        {
            ...
            var result = await _inforVisualService.GetSameDayReceivingQuantityAsync(
                poNumber,
                partID,
                DateTime.Today
            );
        ...
        public Model_ReceivingValidationResult ValidateReceivingLoad(Model_ReceivingLoad load)
        {
            ...
            if (!load.IsNonPOItem)
            {
                ...
                if (string.IsNullOrWhiteSpace(load.PoLineNumber))
                {
                    errors.Add($"Load {load.LoadNumber}: PO part selection is required");
                }
            }
            ...
            if (
                ...
                && load.InitialLocation.Length > 15
            )
            {
        ...
        public Model_ReceivingValidationResult ValidateSession(List<Model_ReceivingLoad> loads)
        {
            ...
            {
                return Model_ReceivingValidationResult.Error(
                    "Session must contain at least one load"
                );
            }
            ...
            {
                var result = Model_ReceivingValidationResult.Error(
                    $"{allErrors.Count} validation error(s) found"
                );
        ...
        public async Task<Model_ReceivingValidationResult> ValidatePartExistsInVisualAsync(
            string partID
        )
        {
            ...
            {
                return Model_ReceivingValidationResult.Error(
                    $"Error validating part: {result.ErrorMessage}"
                );
            }
            ...
            {
                return Model_ReceivingValidationResult.Error(
                    $"Part ID {partID} not found in Infor Visual database"
                );
            }
