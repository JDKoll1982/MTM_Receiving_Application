...
### ✅ Files Found
- `ViewModels/Receiving/Receiving_WorkflowViewModel.cs` (contains StatusMessage pattern)
- `ViewModels/Receiving/Receiving_ReviewGridViewModel.cs` (not ReceivingReviewViewModel)
- `ViewModels/Receiving/Receiving_PackageTypeViewModel.cs`
- `ViewModels/Receiving/Receiving_LoadEntryViewModel.cs`
- `ViewModels/Dunnage/Dunnage_ReviewViewModel.cs`
- `ViewModels/Dunnage/Dunnage_ModeSelectionViewModel.cs`
...
- All Dunnage Views in `Views/Dunnage/`
- Settings Views: `Settings_WorkflowView.xaml`, `Settings_ModeSelectionView.xaml`, `Settings_DunnageModeView.xaml`, `Settings_PlaceholderView.xaml`

### ❌ Files NOT Found (Need Clarification)
- `ViewModels/Receiving/ReceivingWorkflowViewModel.cs` → Actual: `Receiving_WorkflowViewModel.cs`
- `ViewModels/Dunnage/DunnageWorkflowViewModel.cs` → No single workflow ViewModel exists
- `ViewModels/Receiving/ReceivingReviewViewModel.cs` → Actual: `Receiving_ReviewGridViewModel.cs`
- `ViewModels/Receiving/ReceivingPreferencesViewModel.cs` → Not found (preferences may be in Receiving_PackageTypeViewModel)
- `Views/Settings/MainNavigationView.xaml` → Not found (may be in MainWindow or different naming)
- `Views/Receiving/NumberOfLoadsView.xaml` → Not found (may be `Receiving_LoadEntryView.xaml`)
- `ViewModels/Receiving/NumberOfLoadsViewModel.cs` → Not found (may be `Receiving_LoadEntryViewModel.cs`)
- `Data/Receiving/Dao_PackageType.cs` → Actual: `Dao_PackageTypePreference.cs`
...
- Help button exists in `Receiving_WorkflowView.xaml` with Click="HelpButton_Click" event handler
...
- **Files**: `Receiving_ModeSelectionViewModel.cs`, `Dunnage_ModeSelectionViewModel.cs`
...
- **Receiving Review**: Button text = "**Add Another Part/PO**" (line 281 of Receiving_ReviewGridView.xaml)
  - Command: `AddAnotherPartCommand` in `Receiving_ReviewGridViewModel.cs`
  - Current behavior: Calls `_workflowService.AddCurrentPartToSessionAsync()` then navigates back to POEntry
- **Dunnage Review**: Button text = "**Add Another**" (line 243 of Dunnage_ReviewView.xaml)
  - Command: `AddAnotherCommand` in `Dunnage_ReviewViewModel.cs`
...
- **Settings Views**: Located in `Views/Settings/` directory (Settings_WorkflowView, Settings_ModeSelectionView, Settings_DunnageModeView, Settings_PlaceholderView)
...
**Finding**: `Receiving_PackageTypeViewModel.cs` handles package type preference management.
- **File**: `ViewModels/Receiving/Receiving_PackageTypeViewModel.cs`
...
- **Files with Help Buttons**:
  - `Receiving_WorkflowView.xaml` (line 138) - HelpButton_Click event, calls `IService_Help.ShowContextualHelpAsync(currentStep)`
  - `Dunnage_WorkflowView.xaml` (line 102) - HelpButton_Click event (same pattern)
  - `Main_CarrierDeliveryLabelPage.xaml` (line 8) - HelpButton defined
  - `Dunnage_QuickAddTypeDialog.xaml` (lines 38, 41) - HelpButton with custom style
...
- **Services**: `Services/[Feature]/[Service].cs`
- **Models**: `Models/[Feature]/Model_[Entity].cs`
- **Helpers**: `Helpers/[Category]/[Helper].cs`
...
- File paths need correction: ViewModels use underscore naming convention (e.g., `Receiving_WorkflowViewModel.cs` not `ReceivingWorkflowViewModel.cs`)
...
- Added `IService_Window` dependency injection to both mode selection ViewModels
...
- Implemented `ConfirmModeChangeAsync()` method using `ContentDialog` with XamlRoot from `IService_Window`
...
**Files Modified:**
- `ViewModels/Receiving/Receiving_ModeSelectionViewModel.cs`
- `ViewModels/Dunnage/Dunnage_ModeSelectionViewModel.cs`

- [x] T001 [ModeSelect] Update `ViewModels/Receiving/Receiving_ModeSelectionViewModel.cs` and `ViewModels/Dunnage/Dunnage_ModeSelectionViewModel.cs` to add confirmation dialog in mode select command
...
- [x] T003 [ModeSelect] Add user prompt using ContentDialog or `IService_ErrorHandler.ShowErrorDialogAsync()` with custom buttons with message "This will reset all unsaved data. Continue?"
  ...
  - [x] T003b: If using ContentDialog, determine where to get XamlRoot context (from View or via IService_Window)
  - **IMPLEMENTED**: Used `ContentDialog` with `IService_Window.GetXamlRoot()` for XamlRoot context
...
- [x] T005 [DunnageViews] Analyze notification area logic in `ViewModels/Receiving/Receiving_WorkflowViewModel.cs` (lines 116-132: ShowStatus method with StatusMessage property, InfoBarSeverity, and 5-second auto-dismiss using Task.Delay)
  - **PATTERN CONFIRMED**: `ShowStatus()` method implementation found
  - **BASE CLASS**: All ViewModels inherit from `Shared_BaseViewModel` which provides `StatusMessage` property (line 18)
...
- [ ] T006 [P] [DunnageViews] Update Dunnage ViewModels to include StatusMessage, StatusSeverity, IsStatusOpen properties and ShowStatus method
  - **STATUS**: MOSTLY COMPLETE - All Dunnage ViewModels already use `StatusMessage` property via `Shared_BaseViewModel` inheritance
  ...
  - [ ] T006a: Add to `Dunnage_ModeSelectionViewModel.cs`
  - [ ] T006b: Add to `Dunnage_TypeSelectionViewModel.cs`
  - [ ] T006c: Add to `Dunnage_PartSelectionViewModel.cs`
  - [ ] T006d: Add to `Dunnage_DetailsEntryViewModel.cs`
  - [ ] T006e: Add to `Dunnage_QuantityEntryViewModel.cs`
  - [ ] T006f: Add to `Dunnage_ReviewViewModel.cs`
  - [ ] T006g: Add to `Dunnage_ManualEntryViewModel.cs`
  - [ ] T006h: Add to `Dunnage_EditModeViewModel.cs`
  - [ ] T006i: Add to `Dunnage_AdminMainViewModel.cs`, `Dunnage_AdminTypesViewModel.cs`, `Dunnage_AdminPartsViewModel.cs`, `Dunnage_AdminInventoryViewModel.cs` (if notifications are needed in admin views)
- [ ] T007 [P] [DunnageViews] Update Dunnage Views to include InfoBar notification area UI matching Receiving workflow
  - [ ] T007a: Add InfoBar to `Dunnage_ModeSelectionView.xaml`
  - [ ] T007b: Add InfoBar to `Dunnage_TypeSelectionView.xaml`
  - [ ] T007c: Add InfoBar to `Dunnage_PartSelectionView.xaml`
  - [ ] T007d: Add InfoBar to `Dunnage_DetailsEntryView.xaml`
  - [ ] T007e: Add InfoBar to `Dunnage_QuantityEntryView.xaml`
  - [ ] T007f: Add InfoBar to `Dunnage_ReviewView.xaml`
  - [ ] T007g: Add InfoBar to `Dunnage_ManualEntryView.xaml`
  - [ ] T007h: Add InfoBar to `Dunnage_EditModeView.xaml`
- [ ] T008 [DunnageViews] Implement auto-dismiss logic using Task.Delay(5000) pattern (not DispatcherTimer) in all Dunnage ViewModels with IService_Dispatcher dependency injection
  ...
  - [ ] T008b: Copy exact ShowStatus implementation from Receiving_WorkflowViewModel.cs
...
**⚠️ VALIDATION ISSUES FOUND:**
- File `Views/Settings/MainNavigationView.xaml` does not exist - actual settings files use different naming pattern (e.g., `Settings_ModeSelectionView.xaml`)
...
- [ ] T012 [SettingsViews] Update main headers in Settings Views to include Material Design icons matching Dunnage and Receiving workflows
  - [ ] T012a: Add icon to `Views/Settings/Settings_WorkflowView.xaml` (if not already present)
  - [ ] T012b: Add icon to `Views/Settings/Settings_PlaceholderView.xaml` (if not already present)
  - [ ] T012c: Verify `Settings_ModeSelectionView.xaml` and `Settings_DunnageModeView.xaml` already use Material.Icons (validation shows they do)
...
- [ ] T013 [SettingsViews] Fix background coloring in all Settings Views to match Dunnage and Receiving workflow backgrounds
  ...
  - [ ] T013b: Apply same background to `Settings_WorkflowView.xaml`, `Settings_ModeSelectionView.xaml`, `Settings_DunnageModeView.xaml`, `Settings_PlaceholderView.xaml`
...
**⚠️ VALIDATION ISSUES FOUND:**
- File `ReceivingReviewViewModel.cs` does not exist - actual file is `Receiving_ReviewGridViewModel.cs`
- File `DunnageReviewViewModel.cs` exists as `Dunnage_ReviewViewModel.cs`
- Need to identify the exact "Plus New Item" button and its command

**✅ IMPLEMENTATION COMPLETE (2026-01-03)**
- Added `IService_Window` dependency injection to both Review ViewModels
...
**Files Modified:**
- `ViewModels/Receiving/Receiving_ReviewGridViewModel.cs`
- `ViewModels/Dunnage/Dunnage_ReviewViewModel.cs`

- [x] T015 [ReviewPage] Update "Add Another Part/PO" button command in `ViewModels/Receiving/Receiving_ReviewGridViewModel.cs` to clear data on related views before adding new item
...
- [x] T016 [ReviewPage] Update "Add Another" button command in `ViewModels/Dunnage/Dunnage_ReviewViewModel.cs` to clear data on related views before adding new item
...
- [x] T017 [ReviewPage] Implement data clearing logic that resets form fields and collections in dependent ViewModels
  ...
  - **ENHANCEMENT NEEDED**: Extend `ClearTransientWorkflowData()` to access and clear ViewModel properties:
    - **Receiving ViewModels to Clear**:
      - `Receiving_POEntryViewModel`: Reset `PoNumber=""`, `PartID=""`, `SelectedPart=null`, `IsNonPOItem=false`, `Parts.Clear()`
      - `Receiving_PackageTypeViewModel`: Reset `SelectedPackageType=""`, `CustomPackageTypeName=""`, `IsCustomTypeVisible=false`
      - `Receiving_LoadEntryViewModel`: Reset `NumberOfLoads=1`
      - `Receiving_WeightQuantityViewModel`: Clear `WeightQuantity` values in loads collection
      - `Receiving_HeatLotViewModel`: Clear `HeatLotNumber` values in loads collection
    - **Dunnage ViewModels to Clear**:
      - `Dunnage_TypeSelectionViewModel`: Reset `SelectedType=null`
      - `Dunnage_PartSelectionViewModel`: Reset `SelectedPart=null`
      - `Dunnage_DetailsEntryViewModel`: Reset `PONumber=""`, `Location=""`, `SpecInputs.Clear()`
      - `Dunnage_QuantityEntryViewModel`: Reset `Quantity=1`
...
- [ ] T019 [ReceivingWorkflow] Investigate "failed to save preference" error when saving custom package type names
  - [x] T019a: **CONFIRMED** - `Receiving_PackageTypeViewModel.cs` handles preferences (line 168-194: SavePreferenceAsync method)
...
- [ ] T021 [ReceivingWorkflow] Verify help content availability (help button infrastructure exists)
  - [x] T021a: **CONFIRMED** - Help button exists in `Receiving_WorkflowView.xaml` line 138 with HelpButton_Click event
  - [x] T021b: **CONFIRMED** - `Receiving_WorkflowView.xaml.cs` line 24-31 calls `_helpService.ShowContextualHelpAsync(_workflowService.CurrentStep)`
  ...
  - [x] T021d: **CONFIRMED** - Dunnage also has help button in `Dunnage_WorkflowView.xaml` line 102 with same pattern
- [ ] T022 [ReceivingWorkflow] Add or fix help content for workflow steps
  - [ ] T022a: Review `IService_Help` implementation to see how ShowContextualHelpAsync works and where content is stored
    - **RESEARCH NEEDED**: Locate `IService_Help` interface in `Contracts/Services/` directory
    - **RESEARCH NEEDED**: Find implementation class (likely `Service_Help` in `Services/Help/`)
    - **RESEARCH NEEDED**: Determine content storage format (JSON, XML, database, or hardcoded strings)
    - **REFERENCE**: Known usage in `Receiving_WorkflowView.xaml` line 138: `_helpService.ShowContextualHelpAsync(currentStep)`
...
- "Start New Entry" button exists in `Receiving_WorkflowView.xaml` (line 105) on completion screen
- Command: `StartNewEntryCommand` in `Receiving_WorkflowViewModel.cs` (line 219)
...
- Command: `ResetCSVCommand` in `Receiving_WorkflowViewModel.cs` (line 226)
- Heat/Lot# implementation: `Receiving_HeatLotViewModel.cs` has `PrepareHeatLotFields()` method
- Default value logic exists in `Receiving_ManualEntryViewModel.cs` (line 269) and `Receiving_EditModeViewModel.cs` (line 731)
...
1. **Receiving_POEntryViewModel.cs**:
...
2. **Receiving_PackageTypeViewModel.cs**:
...
1. **Dunnage_TypeSelectionViewModel.cs**:
   - `SelectedType` (Model_DunnageType?) - Selected dunnage type

2. **Dunnage_PartSelectionViewModel.cs**:
   - `SelectedPart` (Model_DunnagePart?) - Selected part

3. **Dunnage_DetailsEntryViewModel.cs**:
   - `PONumber` (string) - PO number input
...
- [ ] T025 [P] [SaveWorkflow] Separate CSV and Database save operations in Dunnage workflow
  - [ ] T025a: Review current implementation in `Service_DunnageWorkflow.SaveSessionAsync()` (lines 138-193) and `Dunnage_ReviewViewModel.SaveAllAsync()` (lines 189-232)
    - **FOUND**: Service method at `Services/Receiving/Service_DunnageWorkflow.cs` lines 138-193
    - **WORKFLOW**: Dunnage save is triggered from `Dunnage_ReviewViewModel.SaveAllAsync()` which calls both database and CSV operations
  ...
  - [ ] T025d: Update `Dunnage_ReviewViewModel.SaveAllAsync()` to use separated methods
...
- [ ] T026 [ReviewPage] Update "+ Add Another Part/PO" button to save current data to CSV before clearing
  - [ ] T026a: Modify `AddAnotherPartAsync()` in `Receiving_ReviewGridViewModel.cs` (current line 145)
...
- [ ] T027 [ReviewPage] Update "+ Add Another" button (Dunnage) to save current data to CSV before clearing
  - [ ] T027a: Modify `AddAnotherAsync()` in `Dunnage_ReviewViewModel.cs` (current line 174)
...
- [ ] T028 [SaveComplete] Update "Start New Entry" button to reset all local data (not just workflow)
  - [ ] T028a: Review current implementation in `StartNewEntryAsync()` in `Receiving_WorkflowViewModel.cs` (line 219)
    - **CURRENT IMPLEMENTATION**: Calls `_workflowService.ResetWorkflowAsync()` only
...
- [ ] T029 [SaveComplete] Implement "Start New Entry" functionality for Dunnage workflow
  ...
  - [ ] T029b: Create `StartNewEntryCommand` in `Dunnage_WorkflowViewModel` (if exists) or relevant ViewModel
...
- [ ] T030 [HeatLot] Ensure Heat/Lot# defaults to "Nothing Entered" when empty across all workflows
  - [x] T030a: **FOUND** - `Receiving_ManualEntryViewModel.cs` line 269 sets default: `load.HeatLotNumber = "Nothing Entered";`
  - [x] T030b: **FOUND** - `Receiving_EditModeViewModel.cs` line 731 sets default: `load.HeatLotNumber = "Nothing Entered";`
  - [x] T030c: **FOUND** - `Receiving_HeatLotViewModel.cs` has `PrepareHeatLotFields()` method (line 148)
  - [ ] T030d: Verify `PrepareHeatLotFields()` implementation sets "Nothing Entered" for empty fields
    - **FOUND**: Method at `ViewModels/Receiving/Receiving_HeatLotViewModel.cs` lines 150-159
...
- [ ] T032 [ResetCSV] Implement "Reset CSV" button functionality in Dunnage workflow (currently missing)
  - [ ] T032a: Add "Reset CSV" button to `Dunnage_WorkflowView.xaml` (verify file exists or identify correct view)
...
- [ ] T033 [ResetCSV] Update "Reset CSV" button to trigger database save before deleting CSV files
  - [ ] T033a: Review current `ResetCSVAsync()` implementation in `Receiving_WorkflowViewModel.cs` (line 226)
...
|-------------------------------|-------------------|
| `ReceivingWorkflowViewModel.cs` | `Receiving_WorkflowViewModel.cs` |
| `DunnageWorkflowViewModel.cs` | No single file (multiple step ViewModels) |
| `ReceivingReviewViewModel.cs` | `Receiving_ReviewGridViewModel.cs` |
| `DunnageReviewViewModel.cs` | `Dunnage_ReviewViewModel.cs` |
| `ReceivingPreferencesViewModel.cs` | Likely `Receiving_PackageTypeViewModel.cs` |
| `NumberOfLoadsViewModel.cs` | Likely `Receiving_LoadEntryViewModel.cs` |
...
- `IService_Window` - For window management

### Notification Pattern (from Receiving_WorkflowViewModel.cs)
```csharp
[ObservableProperty]
...
4. `Dunnage_DetailsEntryViewModel.cs`
5. `Dunnage_QuantityEntryViewModel.cs`
6. `Dunnage_ReviewViewModel.cs`
7. `Dunnage_ManualEntryViewModel.cs`
8. `Dunnage_EditModeViewModel.cs`
9. `Dunnage_AdminMainViewModel.cs` (admin)
10. `Dunnage_AdminTypesViewModel.cs` (admin)
11. `Dunnage_AdminPartsViewModel.cs` (admin)
12. `Dunnage_AdminInventoryViewModel.cs` (admin)

### All Dunnage Views (Phase 2, T007)
1. `Dunnage_ModeSelectionView.xaml`
2. `Dunnage_TypeSelectionView.xaml`
3. `Dunnage_PartSelectionView.xaml`
4. `Dunnage_DetailsEntryView.xaml`
5. `Dunnage_QuantityEntryView.xaml`
6. `Dunnage_ReviewView.xaml`
7. `Dunnage_ManualEntryView.xaml`
8. `Dunnage_EditModeView.xaml`

### All Settings Views (Phase 3)
1. `Settings_WorkflowView.xaml`
2. `Settings_ModeSelectionView.xaml`
3. `Settings_DunnageModeView.xaml`
4. `Settings_PlaceholderView.xaml`
