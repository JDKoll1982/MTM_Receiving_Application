...
If continuing would require a **major assumption**, do not proceed silently and do not create an assumption file by default. Instead, use the same chat-facing `vscode_askQuestions` approval flow defined in `.github/instructions/workflow/prompt-engineer-every-message.instructions.md`.
...
2. Load the current spec file. **Note**: The spec contains WORKFLOW_DATA blocks (structured key-value format in HTML comments) that define workflows. These are NOT Mermaid diagrams - they are parsed by MermaidGenerator.ps1. When analyzing workflows, read the WORKFLOW_DATA blocks to understand:
   - WORKFLOW: ID and numbering
   - DEPENDS_ON/CONFLICTS_WITH: Workflow relationships
   - INTERACTION: How workflows relate to each other
   - NODE/TYPE/SHAPE/LABEL: Workflow step details
   ...
   Interaction & UX Flow:
   - Critical user journeys / sequences (check WORKFLOW_DATA blocks for completeness)
   - Workflow dependencies and conflicts (validate DEPENDS_ON/CONFLICTS_WITH fields)
