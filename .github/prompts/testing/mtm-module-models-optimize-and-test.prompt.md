...
- Prefer MCP tooling per `.github/instructions/tooling/mcp-tooling.instructions.md`.
...
4. If needed, add/update a defaults file under `<MODULE_FOLDER>/Defaults/` and update the model to consume it.
5. Update or create the corresponding unit test file in `MTM_Receiving_Application.Tests`:
   - Use xUnit + FluentAssertions.
   - Match existing test folder conventions.
   - Ensure tests validate the new behavior or guard against regressions.
6. Run tests (`dotnet test`) and report results:
