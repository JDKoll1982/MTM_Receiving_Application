...
- **FR-017**: ViewModel MUST load all types from Service_MySQL_Dunnage on initialization
...
- **FR-027**: ViewModel MUST load parts for selected type using GetPartsByTypeAsync
- **FR-028**: ViewModel MUST call IsPartInventoriedAsync when part is selected
...
- **FR-042**: ViewModel MUST load spec definitions from selected part's type using GetSpecsForTypeAsync
...
- **FR-056**: Save operation MUST insert loads to database via Service_MySQL_Dunnage. SaveLoadsAsync
- **FR-057**: Save operation MUST export CSV via Service_DunnageCSVWriter.WriteToCSVAsync
