...
const MockData = {
  ...
  materialHandlerWaitlist: [
    ...
    {
      id: 8,
      requestType: "qc-inspection-request",
      category: "Quality Control",
      taskType: "Inspection",
      taskTypeColor: "task-quality-control",
      location: "Press 6",
      zone: "Zone C",
      partNumber: "23-23415-006",
      workOrder: "WO-12345",
    ...
    {
      id: 9,
      requestType: "st-die-issue-stuck",
      category: "Setup Technician",
      taskType: "Die Issue",
      taskTypeColor: "task-setup-tech",
      location: "Press 1",
    ...
    {
      id: 10,
      requestType: "pl-production-question",
      category: "Production Lead",
      taskType: "Question",
      taskTypeColor: "task-production-lead",
      location: "Press 3",
      zone: "Zone B",
      partNumber: null,
      workOrder: "WO-12399",
  ...
  analytics: {
    ...
    handlerPerformance: [
      { name: "Mike L.", tasksCompleted: 14, avgTime: 12.5, zone: "A" },
      { name: "Mike S.", tasksCompleted: 12, avgTime: 15.2, zone: "B" },
      { name: "Tim R.", tasksCompleted: 9, avgTime: 13.8, zone: "C" }
    ],
    pressPerformance: [
  ...
  materialHandlers: [
    { id: 1, name: "Mike L.", zone: "Zone A", status: "Active", tasksToday: 14 },
    { id: 2, name: "Mike S.", zone: "Zone B", status: "Active", tasksToday: 12 },
    { id: 3, name: "Tim R.", zone: "Zone C", status: "Active", tasksToday: 9 }
  ],
  
  // Material Handler Leads
  materialHandlerLeads: [
    { id: 10, name: "Doyle H.", status: "Active" },
    { id: 11, name: "Todd T.", status: "Active" },
    { id: 12, name: "Matt D.", status: "Active" }
  ],
  ...
  // Setup Technicians
  setupTechnicians: [
     { id: 20, name: "Austin P.", status: "Active" },
     { id: 21, name: "John B.", status: "Active" }
  ],

  // Setup Leads
  setupLeads: [
     { id: 30, name: "Cody C.", status: "Active" },
     { id: 31, name: "Bailey P.", status: "Active" },
     { id: 32, name: "Kevin S.", status: "Active" }
  ],

  // Quality Control
  qualityControl: [
     { id: 40, name: "Nou L.", status: "Active" },
     { id: 41, name: "Austin D.", status: "Active" }
  ],

  // Quality Lead
  qualityLeads: [
     { id: 45, name: "Kurt W.", status: "Active" }
  ],

  // Production Leads
  productionLeads: [
     { id: 50, name: "Monica H.", status: "Active" },
     { id: 51, name: "Jennifer M.", status: "Active" }
  ],

  // Plant Manager
  plantManager: { id: 60, name: "Nick W." },
...
const MockUtils = {
  ...
  // Get all request types that a specific role can see (for filtering waitlists)
  getRequestTypesForRole(role) {
    let categories = [];
    
    switch(role) {
      case 'Operator':
        // Operators can create requests for all categories
        categories = ['Material Handler', 'Setup Technician', 'Production Lead', 'Quality Control'];
        break;
      case 'MaterialHandler':
        // Material Handlers see ONLY Material Handler requests
        categories = ['Material Handler'];
        break;
      case 'MaterialHandlerLead':
        // Material Handler Leads see Material Handler + Quality requests
        categories = ['Material Handler', 'Quality Control'];
        break;
      case 'OperatorLead':
      case 'ProductionLead':
        // Production Leads see Production Lead + Material Handler + Quality requests
        categories = ['Production Lead', 'Material Handler', 'Quality Control'];
        break;
      case 'Quality':
      case 'QualityControl':
        // Quality sees ONLY Quality Control requests
        categories = ['Quality Control'];
        break;
      case 'PlantManager':
        // Plant Managers see ALL requests
        categories = ['Material Handler', 'Setup Technician', 'Production Lead', 'Quality Control'];
        break;
      default:
        categories = [];
    ...
    categories.forEach(cat => {
      allTypes = allTypes.concat(this.getRequestTypesByCategory(cat));
    });
    
    return allTypes;
  },

  // Get categories visible to a specific role (for filter dropdowns)
  getCategoriesForRole(role) {
    switch(role) {
      case 'Operator':
        return ['Material Handler', 'Setup Technician', 'Production Lead', 'Quality Control'];
      case 'MaterialHandler':
        return ['Material Handler'];
      case 'MaterialHandlerLead':
        return ['Material Handler', 'Quality Control'];
      case 'OperatorLead':
      case 'ProductionLead':
        return ['Production Lead', 'Material Handler', 'Quality Control'];
      case 'Quality':
      case 'QualityControl':
  ...
  // Filter waitlist by category (role-based)
  filterByCategory(waitlist, category, userRole) {
    // If "All" selected, show all categories visible to this role
    if (!category || category === 'All') {
      const visibleCategories = this.getCategoriesForRole(userRole);
      return waitlist.filter(item => visibleCategories.includes(item.category));
    }
    
    // Otherwise filter to specific category
