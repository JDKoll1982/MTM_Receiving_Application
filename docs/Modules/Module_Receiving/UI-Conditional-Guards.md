...
## Complete Step — Fixed UI Actions

The completion screen now exposes only the actions that remain supported for a finished save:

- `Start New Entry`
- `Clear Label Data`

Back and Next are hidden while the completion screen is visible, and the previous `Allow Edit After Save`
setting is no longer part of the workflow or settings model.
