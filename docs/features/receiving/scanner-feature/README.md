...
- Step 4 (ViewModel behavior) is implemented for the scanner workbench and history flows, including session start, non-blocking add-line validation status/notes mapping, draft item staging, run-snapshot command flow, run-history refresh, workbench send/stop actions, and Manage Items batch editing.
- Step 5 view implementation is now materially present: the workbench, history, and settings views bind to the scanner ViewModel state and commands, and the Manage Items modal now covers add, duplicate, reorder, and delete actions.
