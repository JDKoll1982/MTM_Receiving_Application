...
public sealed class Service_ReceivingShortcutsTests
{
    [Fact]
    public async Task GetShortcutsAsync_ShouldReturnDefaults_WhenSettingsAreMissing()
    {
        ...
        settingsCore
            .Setup(service =>
                service.GetSettingAsync("Receiving", It.IsAny<string>(), It.IsAny<int?>())
            )
    ...
    public async Task SaveShortcutsAsync_ShouldPersistBindingsAndToggle()
    {
        ...
        var service = CreateService(settingsCore.Object);
        var shortcuts = new Model_Settings_ReceivingShortcuts
        {
            ...
            NextStepShortcut = new Model_KeyboardShortcutBinding { Key = "Right" },
            BackStepShortcut = new Model_KeyboardShortcutBinding { Key = "Left" },
        ...
        settingsCore.Verify(
            core =>
                core.SetSettingAsync(
                    ...
                    It.Is<string>(json =>
                        JsonSerializer.Deserialize<Model_KeyboardShortcutBinding>(json)!.Key == "K"
                    ),
                    It.IsAny<int?>()
                ),
            Times.Once
    ...
    private static Service_ReceivingShortcuts CreateService(
        IService_SettingsCoreFacade settingsCore
    )
    {
        return new Service_ReceivingShortcuts(
            settingsCore,
