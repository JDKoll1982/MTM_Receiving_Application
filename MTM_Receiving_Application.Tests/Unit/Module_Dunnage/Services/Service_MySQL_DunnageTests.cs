...
public sealed class Service_MySQL_DunnageTests
{
    ...
    public async Task InsertPartWithInventoryAsync_ShouldInjectNormalizedImagePathIntoSpecJson()
    {
        ...
        var imageStorage = new Mock<IService_DunnageImageStorage>();
        imageStorage
            .Setup(storage => storage.ImportImageAsync(It.IsAny<string>(), "Parts"))
            .ReturnsAsync(Model_Dao_Result_Factory.Success("Parts/test-part.png"));
        imageStorage
            .Setup(storage => storage.GetNormalizedFullPath("Parts/test-part.png"))
            .Returns("\\\\server\\share\\Dunnage\\Parts\\test-part.png");
        ...
        result.IsSuccess.Should().BeTrue();
        part.SpecValues.Should()
            .Contain(
                "\"image_path\":\"\\\\\\\\server\\\\share\\\\Dunnage\\\\Parts\\\\test-part.png\""
            );
        daoPart.Verify(
            dao =>
                dao.InsertWithInventoryAsync(
