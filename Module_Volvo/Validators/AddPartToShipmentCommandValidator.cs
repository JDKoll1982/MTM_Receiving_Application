...
public class AddPartToShipmentCommandValidator : AbstractValidator<AddPartToShipmentCommand>
{
    public AddPartToShipmentCommandValidator()
    {
        ...
        RuleFor(x => x.ReceivedSkidCount)
            .GreaterThan(0)
            .WithMessage("Received skid count must be greater than 0");
        ...
        When(
            ...
            () =>
            {
                RuleFor(x => x.ExpectedSkidCount)
                    .NotNull()
                    .WithMessage("Expected skid count is required when there is a discrepancy")
                    .GreaterThan(0)
                    .WithMessage("Expected skid count must be greater than 0");

                RuleFor(x => x.DiscrepancyNote)
                    .NotEmpty()
                    .WithMessage("Discrepancy note is required when there is a discrepancy")
                    .MaximumLength(500)
                    .WithMessage("Discrepancy note must not exceed 500 characters");
            }
        );
    }
}
