...
The live table has a unique key `uq_settings_weldedcoils_partid` on `partid`, added 2026-09-15 with
`Database/Scripts/DedupeWeldedCoils/`. Duplicate part numbers are rejected by both the unique key and
`sp_Settings_WeldedCoils_Insert`, which reports a friendly error before the constraint is reached.
...
- `Models/Model_Tool_WeldedCoil.cs` — observable row model (`Id`, `PartId`, `IsActive`).
  `IsActive` is kept for table-mapping completeness; the tool no longer toggles it.
...
- `ViewModels/ViewModel_Tool_WeldedCoils.cs` — add / search / per-row delete / delete-selected
  logic with re-entrancy and row-identity guards.
- `Views/View_Tool_WeldedCoils.xaml` (+ code-behind) — tool page: search bar, Add Coil and
  Delete Selected buttons, and a read-only ListView with a Delete button on each row.
...
5. Use a row's **Delete** button to remove just that one part from the list.
...
## Duplicate Rows (Resolved 2026-09-15)

Every part in the list used to render twice or more because the table itself held duplicate rows:
32 rows across 15 distinct part numbers, including one part at four rows. Those rows were written
before the duplicate guard existed, and the table had no unique key to stop more of them.

Resolved by `Database/Scripts/DedupeWeldedCoils/`: the surplus rows were deleted (keeping the lowest
`id` per part number) and `uq_settings_weldedcoils_partid` was added. `sp_Settings_WeldedCoils_GetAll`
now returns 15 rows for the 15 distinct parts. The script is read-only by default, backs the table up
...
1. `Database/Database_Deployment/Sql_Files/Schemas/49_Table_settings_weldedcoils.sql`
   (idempotent `9-15):** deployed and verified on `mtm_receiving_application_test` at
`172.16.1.104:3306` — table created, all five procedures present, and insert / duplicate
rejection / update / set-active / delete verified end-to-end. Live `mtm_receiving_application` also
has the table, all five procedures, and the `partid` unique key; the dedupe run on 2026-09-15 left
the table at 15 rows with a 32-row rollback copy in `settings_weldedcoils_bak_20260915_132241`
