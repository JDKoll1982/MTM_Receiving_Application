...
**Independent Test**: Can be tested by running the seed data script and verifying that 11 dunnage types exist with their default specification schemas (Width, Height, Depth in inches, IsInventoriedToVisual).
...
2. **Given** seed types are created, **When** querying the dunnage_specs table, **Then** each type has a default spec schema with Width, Height, Depth (all in inches), and IsInventoriedToVisual fields
...
- MySQL version is 5.7.24 (no CHECK constraint support at database level - enforced in application layer only)
- Database character set is utf8mb4 with utf8mb4_unicode_ci collation for all text columns
- JSON validation is handled by MySQL's native JSON type (syntax only, not schema)
- Legacy data from `label_table_dunnage` is considered obsolete and not worth migrating
- UUID generation for transaction records is handled by the application layer, not database triggers
- Quantity validation (> 0) requires application-layer enforcement due to MySQL 5.7.24 limitations
- All dimensional measurements (Width, Height, Depth) are standardized to inches - application UI must display unit labels to users
...
- ❌ Application layer DAOs, services, or ViewModels (separate implementation phase)
- ❌ CHECK constraints at database level (MySQL 5.7.24 limitation - enforce in application)
- ❌ Unit conversion functionality - all dimensions stored in inches only (no metric support)
...
folder "Database/Schemas/" {
  file "01_create_receiving_tables.sql" as f01 #ffcccc
  note right of f01
    **MODIFIED**
  ...
  file "06_create_dunnage_tables.sql" as f06new #ccffcc
  note right of f06new
    **NEW**
    Main migration script
    - DROP IF EXISTS statements
    - CREATE TABLE statements
    - All 5 dunnage tables
    - Foreign key constraints
    - Indexes
  end note
  
  file "06_seed_dunnage_data.sql" as f06seed #ccffcc
  note right of f06seed
    **NEW**
    Seed data script
    - 11 default dunnage types
    - Default spec schemas
    - Width/Height/Depth (inches)
    - IsInventoriedToVisual flag
  end note
}
...
- Q: Should default spec schemas include units of measurement for dimensional fields (Width, Height, Depth), providing clearer data consistency and validation rules? → A: No - keep generic field names ("Width", "Height", "Depth") and handle unit standardization at application layer. All dimensional measurements use inches as the standard unit, and the UI must clearly state this to users.
