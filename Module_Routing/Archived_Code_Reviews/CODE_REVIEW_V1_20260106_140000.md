...
- [x] **RoutingManualEntryViewModel.cs**: `InitializeAsync` call pattern. Ensure this is invoked by the NavigationService or `OnNavigateDataTransferObjects` handler in the View code-behind (or partial `Page` class), as constructors cannot run async code.
