...
$stepKeywordMap = @{
    viewmodel = @("Command", "Async", "Load", "Save", "Add", "Remove", "Update", "Initialize", "Submit", "Search", "Filter", "Validate", "Preview", "Generate", "Complete", "Navigate", "Open", "Close", "On")
    handler   = @("Handle", "Execute", "Run", "Get", "Save", "Update", "Add", "Remove", "Delete", "Generate", "Complete")
    command   = @("Handle", "Execute", "Run", "Save", "Update", "Add", "Remove", "Delete", "Generate", "Complete")
    query     = @("Handle", "Execute", "Get", "Load", "Fetch", "Search", "Filter", "List")
    dao       = @("Get", "Insert", "Update", "Delete", "Save", "Load", "Map", "Fetch")
    test      = @("Should", "When", "Given", "Then")
    xaml      = @("Click", "Tapped", "Loaded", "SelectionChanged", "TextChanged")
}
...
function Normalize-SpacedLetters {
    ...
    if ($tokens.Count -ge 4 -and $shortTokens.Count -ge [math]::Ceiling($tokens.Count * 0.6)) {
        return ($tokens -join "")
    }
...
function Convert-CamelCaseToWords {
    ...
    $cleaned = Normalize-SpacedLetters -Value $Value
    $withSpaces = $cleaned -replace "([a-z0-9])([A-Z])", '$1 $2'
    $converted = ($withSpaces -replace "_", " ")
    $collapsed = ($converted -replace "\s+", " ").Trim()

    if ($Value -notmatch "\s" -and (($collapsed -replace " ", "") -eq $Value)) {
        return $Value
    }
...
function Get-DecisionConditions {
    ...
    $matches = [regex]::Matches($Content, $ifConditionPattern)
    $conditions = @()
    foreach ($match in $matches) {
        $conditions += $match.Groups[1].Value.Trim()
    }
...
function Get-EnumMembers {
    ...
    $enumMatch = [regex]::Match($Content, $enumMemberPattern)
    if (-not $enumMatch.Success) {
        return @()
    }

    $body = $enumMatch.Groups[1].Value
    $members = $body -split ',' | ForEach-Object { $_.Trim() } | Where-Object { $_ -match '^[A-Za-z0-9_]+' }
...
function Build-MermaidDiagram {
    ...
    foreach ($step in $Steps) {
        ...
        $label = Convert-MermaidLabel -Value (Convert-CamelCaseToWords -Value $step)
        $lines += "    $nodeId[$label]"
    ...
    foreach ($step in $Steps) {
        ...
        if ($decisionList.Length -ge $nodeIndex - 1) {
            ...
            if ($decisionText) {
                ...
                $lines += ('    {0}["Yes path"]' -f $yesId)
                $lines += ('    {0}["No path"]' -f $noId)
                $lines += "    $decisionId -->|Yes| $yesId"
                $lines += "    $decisionId -->|No| $noId"
...
function Build-UserFriendlySteps {
    ...
    foreach ($step in $stepList) {
        $words = Convert-CamelCaseToWords -Value $step
        $lines += "$index. $words."
        $index++
    }
...
function Build-RequiredInfoRows {
    ...
    foreach ($step in $stepList) {
        $words = Convert-CamelCaseToWords -Value $step
        $rows += "| $words | Invoke $words | n/a | n/a | Method: $step | See implementation | "
    }
...
function Get-GeneratedWorkflowContent {
    ...
    $section = Find-WorkflowSection -Index $Index -WorkflowName $WorkflowName -ModuleFolder $ModuleFolder
    if (-not $section -and $ModuleFolder) {
        $section = Find-WorkflowSection -Index $Index -WorkflowName $WorkflowName -ModuleFolder $null
    }
    ...
    $content = if ($section.Content) { $section.Content } else { "" }
    $steps = @()
    switch ($WorkflowType) {
        ...
        "enum" { $steps = Get-EnumMembers -Content $content }
    ...
    $decisions = Get-DecisionConditions -Content $content
