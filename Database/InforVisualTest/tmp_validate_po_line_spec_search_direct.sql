...
WITH CandidateSpecs AS (
    ...
    FROM Eligible
    ORDER BY SqlMatchWeight DESC, PONumber DESC, POLineNumber, BinaryType
)
SELECT
    e.PONumber,
    e.POLineNumber,
    ...
    e.SqlMatchWeight,
    CASE WHEN EXISTS (
        SELECT 1 FROM RankedTop rt
        WHERE rt.PONumber = e.PONumber
          AND rt.POLineNumber = e.POLineNumber
          AND rt.BinaryType = e.BinaryType
    ) THEN 'YES' ELSE 'NO' END AS InTopMaxResults,
    LEFT(e.SpecText, 120) AS SpecPreview
FROM Eligible e
WHERE e.PONumber = 'PO-070945' AND e.POLineNumber = 1
ORDER BY e.SqlMatchWeight DESC, e.BinaryType;
