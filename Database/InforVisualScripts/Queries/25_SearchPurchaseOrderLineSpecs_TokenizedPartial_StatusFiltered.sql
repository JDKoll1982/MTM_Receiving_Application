/*
Tokenized-partial PO line spec search with required PO status filter.
...
SELECT TOP (@MaxResults)
    ...
    SupplementalText,
    CASE
        WHEN SpecSearchText LIKE @NormalizedWildcard THEN 240
        WHEN SupplementalText LIKE @NormalizedWildcard THEN 180
        WHEN SpecSearchText LIKE @FirstTokenLike THEN 120
        WHEN SupplementalText LIKE @FirstTokenLike THEN 90
        ELSE 0
    END AS SqlMatchWeight
FROM CandidateSpecs
WHERE
    PoStatus = @PoStatusCode
    AND (
        SpecSearchText LIKE @NormalizedWildcard
