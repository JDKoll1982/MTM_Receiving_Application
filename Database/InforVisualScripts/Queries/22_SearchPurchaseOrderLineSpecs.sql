/*
Searches purchase-order line specification content stored in PURC_LINE_BINARY
and related PO line spec-like text fields.

Parameters expected from C#:
  @SearchTerm          NVARCHAR(200)  -- normalized uppercase term with collapsed spaces
  @SearchLike          NVARCHAR(260)  -- '%' + @SearchTerm + '%'
  @NormalizedWildcard  NVARCHAR(260)  -- token wildcard pattern, e.g. '%PUR%AIR%CYLINDER%'
  @FirstTokenLike      NVARCHAR(260)  -- '%' + first token + '%'
  @MaxResults          INT
*/
...
SELECT TOP (@MaxResults)
    ...
    SupplementalText,
    CASE
        WHEN SpecSearchText LIKE @SearchLike THEN 300
        WHEN SpecSearchText LIKE @NormalizedWildcard THEN 250
        WHEN SupplementalText LIKE @SearchLike THEN 200
        WHEN SupplementalText LIKE @NormalizedWildcard THEN 170
        WHEN SpecSearchText LIKE @FirstTokenLike THEN 100
...
WHERE
    ...
    OR SpecSearchText LIKE @NormalizedWildcard
    OR SupplementalText LIKE @SearchLike
    OR SupplementalText LIKE @NormalizedWildcard
    OR SpecSearchText LIKE @FirstTokenLike
