...
CREATE TABLE IF NOT EXISTS volvo_part_components (
    ...
    parent_part_number VARCHAR(20) NOT NULL COMMENT 'Parent part number (references volvo_parts_master.part_number)',
    component_part_number VARCHAR(20) NOT NULL COMMENT 'Component part number (references volvo_parts_master.part_number)',
    ...
    -- Foreign keys: maintain referential integrity with volvo_parts_master
    CONSTRAINT fk_volvo_part_components_parent
        FOREIGN KEY (parent_part_number)
        REFERENCES volvo_parts_master(part_number)
        ON DELETE CASCADE,
