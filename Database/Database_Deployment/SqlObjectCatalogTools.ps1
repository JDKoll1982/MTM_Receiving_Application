...
. (Join-Path $PSScriptRoot 'SqlCodebaseReferenceTools.ps1')
...
function Get-SqlFileCategory {
    ...
    if ($normalizedPath -match '/Sql_Files/Schemas/') {
        return 'schema_file'
    }

    if ($normalizedPath -match '/Sql_Files/Migrations/') {
        return 'migration_file'
    }

    if ($normalizedPath -match '/Sql_Files/Views/') {
        return 'view_file'
    }

    if ($normalizedPath -match '/Sql_Files/StoredProcedures/') {
        return 'stored_procedure_file'
    }

    if ($normalizedPath -match '/Sql_Files/Triggers/') {
        return 'trigger_file'
    }

    if ($normalizedPath -match '/Sql_Files/SeedData/') {
        return 'seed_file'
    }

    if ($normalizedPath -match '/Sql_Files/Scripts/') {
        return 'script_file'
    }
...
function Test-SqlEphemeralMigrationRoutine {
    ...
    return $Entry.objectType -eq 'stored_procedure' -and $Entry.category -eq 'migration_file'
}
...
function Get-SqlAliasMap {
    ...
    $updateAliasMatches = [regex]::Matches(
        $SqlText,
        'UPDATE\s+(?:`?(?<schema>[A-Za-z0-9_]+)`?\.)?`?(?<table>[A-Za-z0-9_]+)`?(?:\s+(?:AS\s+)?(?<alias>[A-Za-z0-9_]+))?(?=\s+(?:INNER|LEFT|RIGHT|CROSS|JOIN|SET)\b)',
        [System.Text.RegularExpressions.RegexOptions]::IgnoreCase
    )

    foreach ($match in $updateAliasMatches) {
        $tableName = Resolve-SqlObjectName -SchemaName $match.Groups['schema'].Value -ObjectName $match.Groups['table'].Value
        $aliasName = $match.Groups['alias'].Value
    ...
    $derivedAliasMatches = [regex]::Matches(
        $SqlText,
        '(?:FROM|JOIN)\s*\(\s*SELECT[\s\S]*?\)\s*(?:AS\s+)?(?<alias>[A-Za-z0-9_]+)\b',
        [System.Text.RegularExpressions.RegexOptions]::IgnoreCase
    )
...
function ConvertTo-SqlCatalogEntry {
    ...
    if ($objectType -eq 'table') {
        return [ordered]@{
            ...
            codebaseReferenceCount     = 0
            codebaseReferenceFileCount = 0
            codebaseReferences         = @()
            parameters                 = @()
            declaredVariables          = @()
            referencedObjects          = @()
...
function Get-TrackedSqlFiles {
    ...
    $includeRoots = @(
        ...
        (Join-Path $DatabaseRoot 'Migrations'),
        (Join-Path $DatabaseRoot 'Views'),
        (Join-Path $DatabaseRoot 'StoredProcedures'),
        (Join-Path $DatabaseRoot 'Triggers'),
...
function New-SqlObjectCatalog {
    ...
    $storedProcedureNames = @(
        $entries |
        Where-Object { $_.objectType -eq 'stored_procedure' -and -not [string]::IsNullOrWhiteSpace($_.objectName) } |
        ForEach-Object { $_.objectName } |
        Sort-Object -Unique
    )

    $codebaseReferenceIndex = Get-CodebaseStoredProcedureReferenceIndex -RepoRoot $RepoRoot -ProcedureNames $storedProcedureNames

    foreach ($entry in $entries) {
        if ($entry.objectType -ne 'stored_procedure' -or [string]::IsNullOrWhiteSpace($entry.objectName)) {
            continue
        }

        if (-not $codebaseReferenceIndex.ContainsKey($entry.objectName)) {
            continue
        }

        $referenceInfo = $codebaseReferenceIndex[$entry.objectName]
        $entry.codebaseReferenceCount = $referenceInfo.totalReferenceCount
        $entry.codebaseReferenceFileCount = $referenceInfo.fileCount
        $entry.codebaseReferences = @($referenceInfo.references | ForEach-Object { $_ })
    }
...
function Test-SqlCatalogEntryAgainstSnapshot {
    ...
    $skipParameterTokenValidation = $Entry.objectType -eq 'stored_procedure' -and (($Entry.procedureCount -as [int]) -gt 1 -or (Test-SqlEphemeralMigrationRoutine -Entry $Entry))
    if (-not $skipParameterTokenValidation) {
