...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM package_types WHERE name = p_name AND is_active = TRUE) THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Package type name already exists';
    END IF;
    
    -- Check for duplicate code
    IF EXISTS (SELECT 1 FROM package_types WHERE code = p_code AND is_active = TRUE) THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Package type code already exists';
    END IF;
    
    INSERT INTO package_types (name, code, is_active, created_by)
