# sp_routing_label_get_all

**Category:** Routing
**Parameter Types:** IN,OUT
**Generated:** 2026-01-11 14:13:05

## Usage Summary

**Total Usages:** 1

## Usage Details

| File | Class | Method | Line | Has Callers | Caller Count |
|------|-------|--------|------|-------------|--------------|
| Module_Routing\Data\Dao_RoutingLabel.cs | Dao_RoutingLabel | `GetAllLabelsAsync` | 178 | ✓ | 5 |

## Code Samples

```csharp
// Module_Routing\Data\Dao_RoutingLabel.cs:178
"sp_routing_label_get_all",

```

## Method References

### Dao_RoutingLabel.GetAllLabelsAsync
**Called by (5 references):**
- Same file (1 calls)
- Module_Routing\Interfaces\IDao_RoutingLabel.cs
- Module_Routing\Services\IRoutingService.cs
- Module_Routing\Services\RoutingService.cs
- Module_Routing\ViewModels\RoutingEditModeViewModel.cs


