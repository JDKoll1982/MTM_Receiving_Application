# sp_volvo_shipment_get_pending

**Category:** Volvo
**Parameter Types:** None
**Generated:** 2026-01-11 16:28:36

## Usage Summary

**Total Usages:** 1

## Usage Details

| File | Class | Method | Line | Has Callers | Caller Count |
|------|-------|--------|------|-------------|--------------|
| Module_Volvo\Data\Dao_VolvoShipment.cs | Dao_VolvoShipment | `GetPendingAsync` | 169 | ✓ | 3 |

## Code Samples

```csharp
// Module_Volvo\Data\Dao_VolvoShipment.cs:169
"sp_volvo_shipment_get_pending",

```

## Method References

### Dao_VolvoShipment.GetPendingAsync
**Called by (3 references):**
- Same file (1 calls)
- Module_Volvo\Interfaces\IDao_VolvoShipment.cs
- Module_Volvo\Services\Service_Volvo.cs


