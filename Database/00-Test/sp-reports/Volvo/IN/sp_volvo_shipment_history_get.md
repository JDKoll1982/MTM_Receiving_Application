# sp_volvo_shipment_history_get

**Category:** Volvo
**Parameter Types:** IN
**Generated:** 2026-01-11 13:34:59

## Usage Summary

**Total Usages:** 1

## Usage Details

| File | Class | Method | Line | Has Callers | Caller Count |
|------|-------|--------|------|-------------|--------------|
| Module_Volvo\Data\Dao_VolvoShipment.cs | Dao_VolvoShipment | `GetHistoryAsync` | 253 | ✓ | 6 |

## Code Samples

```csharp
// Module_Volvo\Data\Dao_VolvoShipment.cs:253
"sp_volvo_shipment_history_get",

```

## Method References

### Dao_VolvoShipment.GetHistoryAsync
**Called by (6 references):**
- Module_Core\Contracts\Services\IService_Volvo.cs
- Module_Core\Services\Database\Service_MySQL_Receiving.cs
- Module_Receiving\Data\Dao_ReceivingLoad.cs
- Same file (1 calls)
- Module_Volvo\Services\Service_Volvo.cs
- Module_Volvo\ViewModels\ViewModel_Volvo_History.cs


