# sp_volvo_part_component_get

**Category:** Volvo
**Parameter Types:** IN
**Generated:** 2026-01-11 16:28:36

## Usage Summary

**Total Usages:** 1

## Usage Details

| File | Class | Method | Line | Has Callers | Caller Count |
|------|-------|--------|------|-------------|--------------|
| Module_Volvo\Data\Dao_VolvoPartComponent.cs | Dao_VolvoPartComponent | `GetByParentPartAsync` | 38 | ✓ | 3 |

## Code Samples

```csharp
// Module_Volvo\Data\Dao_VolvoPartComponent.cs:38
"sp_volvo_part_component_get",

```

## Method References

### Dao_VolvoPartComponent.GetByParentPartAsync
**Called by (3 references):**
- Same file (2 calls)
- Module_Volvo\Services\Service_Volvo.cs
- Module_Volvo\Services\Service_VolvoMasterData.cs


