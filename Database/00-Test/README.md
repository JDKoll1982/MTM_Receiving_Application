...
|------|-------------|--------------|
| `01-mock-data.json` | Mock parameter values for each SP | Script 01 |
...
|----------|-------------|----------|-----------------|
| **Schema Broken** | References non-existent columns/tables | 🔴 Critical | Fix SP SQL or database schema |
| **Parameter Mismatch** | Wrong number of arguments | ⚠️ High | Regenerate mock data |
| **Constraint Violation** | Foreign key errors | ⚠️ Medium | Use `-UseExecutionOrder` or add test data |
| **Data Validation** | Type mismatch, truncation, duplicates | ⚠️ Medium | Update mock values in 01-mock-data.json |
| **Runtime Error** | Other execution failures | ℹ️ Low | Investigate individual SPs |
| **Logic Caught** | Intentional SP validation (SQLSTATE 45000) | ✅ Expected | These are normal business logic checks |
