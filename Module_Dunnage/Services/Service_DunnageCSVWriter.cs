...
namespace MTM_Receiving_Application.Module_Dunnage.Services
{
    public class Service_DunnageCSVWriter : IService_DunnageCSVWriter
    {
        ...
        public async Task<Model_Dunnage_Result_CSVWrite> WriteToCSVAsync(List<Model_DunnageLoad> loads)
        {
        ...
        public async Task<Model_Dunnage_Result_CSVWrite> WriteToCsvAsync(List<Model_DunnageLoad> loads, string typeName)
        {
            if (loads == null || loads.Count == 0)
            {
                await _logger.LogWarningAsync("WriteToCsvAsync called with no loads to export");
                return new Model_Dunnage_Result_CSVWrite { ErrorMessage = "No loads to export." };
            }
            ...
            try
            {
                ...
                return new Model_Dunnage_Result_CSVWrite
                {
                    LocalSuccess = true,
            ...
            catch (Exception ex)
            {
                ...
                return new Model_Dunnage_Result_CSVWrite { ErrorMessage = $"Export failed: {ex.Message}" };
            }
        }
        ...
        public async Task<Model_Dunnage_Result_CSVWrite> WriteDynamicCsvAsync(
        ...
        public async Task<Model_Dunnage_Result_CSVWrite> ExportSelectedLoadsAsync(
            List<Model_DunnageLoad> selectedLoads,
            bool includeAllSpecColumns = false)
        {
            if (selectedLoads == null || selectedLoads.Count == 0)
            {
                return new Model_Dunnage_Result_CSVWrite { ErrorMessage = "No loads selected for export." };
            }
        ...
        public async Task<Model_Dunnage_Result_CSVDelete> ClearCSVFilesAsync(string? filenamePattern = null)
        {
            var result = new Model_Dunnage_Result_CSVDelete();
